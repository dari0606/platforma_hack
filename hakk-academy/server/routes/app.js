...
r.get('/tafsir', (req, res) => {
  const rows = all(`SELECT id, name_kk, name_ar, ayah_count, revelation, description, duration_min, teacher_name, youtube_url, status, reviewed_by, is_demo
  ...
  res.json(rows.map((s) => ({ ...s, viewed: viewed.has(s.id), verified: s.status === 'published' && !!s.reviewed_by, reviewed_by: undefined })));
});
...
r.get('/tafsir/:id', (req, res) => {
  ...
  res.json({ surah: { ...s, takeaways: parse(s.takeaways, []), reviewed_by: undefined }, verification: verification(s), ayahs, test, nav, bookmarked: bookmarked(req.user.id, 'surah', s.id) });
});
...
r.get('/words', (req, res) => {
  ...
  const rows = all(`SELECT * FROM words ${where.length ? 'WHERE ' + where.join(' AND ') : ''} ORDER BY id LIMIT 200`, ...p);
...
r.get('/progress', (req, res) => {
  ...
  res.json({
    ...
    attempts, certificates: certs, mistakes: mistakeSummary(u.id), activity,
  });
});
