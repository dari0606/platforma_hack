...
import { QuranReader } from '../components/quran.jsx';
...
export function TafsirCatalog() {
  ...
  return (
    <>
      <PageHead title={t('Тәпсір')} sub="Әмма парасының 37 сүресі: аяттар, мағына аудармасы, аят-аятпен тыңдау, Hakk Academy тәпсір сабақтары және тест.">
        <Link to="/hifz" className="btn ghost">📖 Жаттау прогресі</Link>
      </PageHead>
      <Loader q={q}>{(rows) => {
        const list = rows.filter((r) => (filter === 'all' || (filter === 'video' ? r.youtube_url : filter === 'new' ? !r.viewed : filter === 'hifz' ? r.hifz : r.viewed))
        ...
        return (
          <>
            <div className="row wrap mb">
              ...
              <div className="chips">
                {[['all', 'Барлығы'], ['video', 'Видеосы бар'], ['new', 'Көрілмеген'], ['seen', 'Көрілген'], ['hifz', 'Жаттағандарым']].map(([v, l]) => <button key={v} className={`chip ${filter === v ? 'on' : ''}`} onClick={() => setFilter(v)}>{l}</button>)}
              </div>
            </div>
                ...
                {list.map((r) => (
                  <Link to={`/tafsir/${r.id}`} key={r.id} className="card card-link surah-card">
                    ...
                    <div>
                      ...
                      <div className="small muted">{r.name_meaning && `«${r.name_meaning}» · `}{r.ayah_count} аят · {r.revelation === 'mecca' ? 'Меккелік' : 'Мединелік'}</div>
                      {r.hifz && <span className={`badge ${r.hifz === 'memorized' ? 'green' : 'amber'} mt-sm`}>{r.hifz === 'memorized' ? '✓ Жатталды' : '📖 Жаттап жатырмын'}</span>}
                    </div>
          ...
          <div className="card mb" style={{ background: 'linear-gradient(135deg,#f6f3ff,#fff)' }}>
            <div className="row wrap between" style={{ alignItems: 'flex-start' }}>
              <div>
                ...
                {s.name_meaning && <div className="muted mt-sm">Атауының мағынасы: «{s.name_meaning}»</div>}
            ...
            <div className="row wrap mt">
              ...
              <HifzButtons id={s.id} initial={d.hifz} />
            </div>
          </div>
            ...
            <div className="stack" style={{ gap: 20 }}>
              <div className="card">
                ...
                <div className="grid g3 mt">
                  ...
                  {s.name_meaning && <div className="stat"><div className="l">Атауының мағынасы</div><div className="v" style={{ fontSize: 20 }}>{s.name_meaning}</div></div>}
                </div>
              </div>
              ...
              <div className="card">
                <div className="row between wrap mb"><h2>Аяттар</h2><span className="small muted">{d.ayahs.length} аят · аятты басып тыңдаңыз</span></div>
                <QuranReader surah={s} ayahs={d.ayahs} />
...
export function HifzButtons({ id, initial, onChange }) {
  const [st, setSt] = useState(initial);
  const toast = useToast();
  const set = async (v) => {
    const next = st === v ? null : v;
    const r = await api.post(`/api/hifz/${id}`, { status: next });
    setSt(r.status); onChange?.(r);
    toast(r.status === 'memorized' ? `✓ Жатталды · Әмма парасы: ${r.summary.memorized}/${r.summary.total}` : r.status === 'learning' ? 'Жаттау тізіміне қосылды' : 'Белгі алынды');
  };
  return (
    <div className="seg" role="group" aria-label="Жаттау күйі" style={{ padding: 3 }}>
      <button className={st === 'learning' ? 'on' : ''} onClick={() => set('learning')}>📖 Жаттап жатырмын</button>
      <button className={st === 'memorized' ? 'on' : ''} onClick={() => set('memorized')}>✓ Жаттадым</button>
    </div>
  );
}
