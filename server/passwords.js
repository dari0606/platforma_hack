...
const users = await all("SELECT id, name, email, role FROM users WHERE role != 'student' OR email = 'student@hakk.kz'");
...
const shared = gen();
await run("UPDATE users SET password_hash = ? WHERE role = 'student' AND email LIKE 'student%@hakk.kz' AND email != 'student@hakk.kz'", hashPassword(shared));
