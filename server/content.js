...
async function source(title, extra) {
  return (await get('SELECT id FROM sources WHERE title = ?', title))?.id || insert('sources', { title, ...extra });
}

async function upsertTest(t, links = {}) {
  let test = await get('SELECT id FROM tests WHERE title = ?', t.title);
  if (!test) test = { id: await insert('tests', { title: t.title, category: t.category || 'tajweed', status: 'published', pass_score: 70, ...links }) };
  else await update('tests', test.id, links);
  // never delete existing questions: students' answers and analytics reference them
  if (await get('SELECT 1 FROM questions WHERE test_id = ? LIMIT 1', test.id)) return test.id;
...
export async function importContent({ log = true } = {}) {
  ...
  const srcAr = await source('Tanzil Quran Text (Uthmani)', { author: 'Tanzil Project', book: 'Мұсхаф мәтіні (Хафс ривояты)', url: 'https://tanzil.net', note: juz.source_ar });
  await source('Халифа Алтай — Құран Кәрім қазақша мағына және түсінігі', { author: 'Халифа Алтай', book: 'Құран Кәрім қазақша мағына және түсінігі', note: juz.source_kk });
  const srcProgram = await source('Hakk Academy оқу бағдарламасы', { author: 'Hakk Academy ұстаздары', book: 'Құран оқу және тәжуид курсы (ішкі)' });
  ...
  await tx(async () => {
    ...
    for (const [num, s] of Object.entries(juz.surahs)) {
      ...
      const ex = await get('SELECT * FROM surahs WHERE id = ?', n);
      ...
      } else await update('surahs', n, { name_meaning: meaning, name_kk: ex.is_demo ? kk : ex.name_kk });
      stats.surahs++;
      for (const a of s.ayahs) {
        const row = await get('SELECT id, translation FROM ayahs WHERE surah_id = ? AND number = ?', n, a.n);
        const patch = { global_number: a.g };
        if (!row) await insert('ayahs', { surah_id: n, number: a.n, arabic: a.ar, translation: a.kk, translation_source: 'Халифа Алтай', ...patch });
        else await update('ayahs', row.id, row.translation ? patch : { ...patch, translation: a.kk, translation_source: 'Халифа Алтай' });
        stats.ayahs++;
      }
    ...
    await run(`UPDATE ayahs SET translation_source = 'Халифа Алтай' WHERE translation IS NOT NULL AND translation_source IS NULL`);

    // ---------- knowledge base topics ----------
    const lessonByTag = async (tag) => (await get(`SELECT id FROM lessons WHERE rule_tag = ? AND status = 'published' ORDER BY id LIMIT 1`, tag))?.id || null;
    for (const t of [...TOPICS, ...GUIDES]) {
      ...
      let row = await get('SELECT id, is_demo FROM kb_topics WHERE slug = ?', t.slug);
      if (!row) row = { id: await insert('kb_topics', { slug: t.slug, ...fields, status: 'published', is_demo: 1, source_id: srcProgram, lesson_id: t.rule_tag ? await lessonByTag(t.rule_tag) : null }) };
      else if (row.is_demo) await update('kb_topics', row.id, { ...fields, status: 'published' });
      else continue; // reviewed by a teacher — leave untouched
      if (t.exercises && !(await get('SELECT 1 FROM exercises WHERE topic_id = ? LIMIT 1', row.id))) {
        for (const [i, e] of t.exercises.entries()) await insert('exercises', { topic_id: row.id, type: e.type, prompt: e.prompt, arabic: e.arabic || null, data: exData(e), explanation: e.explanation || null, sort: i + 1 });
      }
      if (t.test) { await upsertTest({ ...t.test, category: 'tajweed' }, { topic_id: row.id }); stats.tests++; }
      stats.topics++;
    }
    for (const t of [...EXTRA_TESTS, ...ARABIC_TESTS]) { await upsertTest(t); stats.tests++; }

    // ---------- Arabic lessons ----------
    for (const block of ARABIC) {
      const course = await get('SELECT id FROM courses WHERE title = ?', block.course);
      if (!course) continue;
      let mod = await get('SELECT id FROM modules WHERE course_id = ? AND title = ?', course.id, block.module);
      if (!mod) mod = { id: await insert('modules', { course_id: course.id, title: block.module, sort: ((await get('SELECT max(sort) m FROM modules WHERE course_id = ?', course.id)).m || 0) + 1 }) };
      for (const l of block.lessons) {
        ...
        let row = await get('SELECT id, is_demo FROM lessons WHERE module_id = ? AND title = ?', mod.id, l.title);
        if (!row) {
          const sort = ((await get('SELECT max(sort) m FROM lessons WHERE module_id = ?', mod.id)).m || 0) + 1;
          row = { id: await insert('lessons', { module_id: mod.id, sort, ...fields, status: 'published', is_demo: 1, source_id: srcProgram }) };
        } else if (row.is_demo) await update('lessons', row.id, { ...fields, status: 'published' });
        else continue;
        if (l.exercises && !(await get('SELECT 1 FROM exercises WHERE lesson_id = ? LIMIT 1', row.id))) {
          for (const [i, e] of l.exercises.entries()) await insert('exercises', { lesson_id: row.id, type: e.type, prompt: e.prompt, arabic: e.arabic || null, data: exData(e), explanation: e.explanation || null, sort: i + 1 });
        }
        stats.lessons++;
      }
    }
    // publish the previously drafted intermediate lessons that now have full content
    await run(`UPDATE lessons SET status = 'published' WHERE is_demo = 1 AND status IN ('draft','review') AND title IN ('Үш әріпті түбір','Атаулы сөйлем')`);

    // ---------- dictionary ----------
    for (const [topic, list] of Object.entries(WORD_TOPICS)) {
      for (const [arabic, translit, meaning, ref] of list) {
        const ex = await get('SELECT id FROM words WHERE arabic = ? AND (topic = ? OR topic IS NULL)', arabic, topic);
        const lvl = topic === 'Етістіктер' ? 'intermediate' : 'beginner';
        const f = { translit, meaning, topic, in_quran: ref ? 1 : 0, quran_ref: ref, level: lvl };
        if (ex) await update('words', ex.id, f); else await insert('words', { arabic, ...f });
        stats.words++;
      }
    }
    await run(`UPDATE words SET topic = 'Әмма парасындағы сөздер' WHERE topic IS NULL AND in_quran = 1`);
    await run(`UPDATE words SET topic = 'Оқу және мектеп' WHERE topic IS NULL`);
  });
